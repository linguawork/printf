...
// ft_printf("|%-08.18d|\n", 34);
	...
	// /*оказалось, что s_hidden это указатель на строку. Я думал,
	...
	// printf("|%.09s|\n", s_hidden); //pft test 90 отработал через warning (-Wall -Wextra -Werror). Отрабатывает с мусором если не дали значение для указателя char *s_hidden.
	...
	// printf("|%*0.c|\n", 12, '-'); // warning: data argument not used by format string [-Wformat-extra-args]
	// ft_printf("|%*0.c|\n", 12, '-');//
...
// b = ft_printf("%-40.23p$\n", NULL);
	...
	/*more % tests:*/
	// printf("%"); // no need to cover this test, pft skips it Warning
	// ft_printf("%");
